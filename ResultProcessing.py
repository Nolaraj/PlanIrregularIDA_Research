#This file intends to process the data in .txt files that were extracted from the STKO POST Processors.
#First data in .txt are extracted and saved to the Excel (Results_Assembly) file and accumulated there.
#Then the data accumulated are then sorted and arranged in order like DriftsX, DriftsY and so On from Group A function.
#Afterwards, the accumulated data are processed and made charts by Group A.
...
def ExcelWriter(ResultFiles, OutputPath, Sheetname):
    ...
    for file in ResultFiles:
        ...
        Earthquakes_fromData.append(SubjectCode[0])
        ScaleFactors_fromData.append(SubjectCode[1])
        Buildings_fromData.append(SubjectCode[2])
        BaseCondtions_fromData.append(SubjectCode[3])
...
def Group_A(Keyword = "Drift X"):
    ...
    for building in Buildings:
        for earthquake in Earthquakes:
            for scalefactor in ScaleFactors_fromData:

                SubjectCode = [building, f"{earthquake}_SF={scalefactor}g"]

                BuildingRows = rowContains_List(worksheet, building)
                EarthquakeRows = rowContains_List(worksheet, earthquake)
                ScaleFactorRows = rowContains_List(worksheet, scalefactor)


                SelectedTable_Rows= List_Intersection(BuildingRows, EarthquakeRows)
                SelectedTable_Rows= List_Intersection(SelectedTable_Rows, ScaleFactorRows)
